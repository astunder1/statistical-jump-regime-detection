...
def main() -> None:
    ...
    if len(training) != TRAIN_LENGTH:
        raise RuntimeError(f"Expected {TRAIN_LENGTH} training rows, found {len(training)}")
    ...
    model.relabel_by_cumulative_return(returns.loc[training.index].to_numpy())
    ...
    switches = test_states[test_states.ne(test_states.shift())]
